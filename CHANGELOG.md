...
- **WSL native host launch validation** - Windows-browser installs from WSL now run a bounded, side-effect-free launch probe before writing browser manifests or registry entries, preventing broken generated wrappers from being published. `surf doctor` runs the same probe only for Surf's canonical, probe-aware wrapper and safely skips legacy or noncanonical paths. Thanks to [@airtonix](https://github.com/airtonix) for the launch diagnosis in [#290](https://github.com/nicobailon/surf-cli/issues/290).
